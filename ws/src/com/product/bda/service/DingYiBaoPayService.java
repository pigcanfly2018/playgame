...
public class DingYiBaoPayService {

	 private static Logger logger=Logger.getLogger(DingYiBaoPayService.class);
		
	    private JdbcTemplate template;
		private String datasource;
			
		public DingYiBaoPayService(JdbcTemplate template,String datasource){
				this.template=template;
				this.datasource=datasource;
		}
		
		public  Long createDybp(String pay_id,String login_name,
				String amount,String ip,
				String pay_method,String remark,String return_url){
			  String sql="insert into mb_dybp(pay_id,login_name,amount,payip,pay_method,remark,"
		...
		public boolean isNotDoDybp(String pay_id){
			 String sql="select count(1) from mb_dybp where state= 0 and pay_id =?  ";
		...
		public String queryLoginname(String pay_id){
			 String sql="select login_name from mb_dybp  where pay_id=? ";
			 Object[] objs=new Object[]{pay_id};
			 logger.info(LogHelper.dbLogObj(datasource,sql,Arrays.asList(objs)));
		...
		public String queryPayType(String pay_id){
			 String sql="select pay_method from mb_dybp  where pay_id=? ";
			 Object[] objs=new Object[]{pay_id};
			 logger.info(LogHelper.dbLogObj(datasource,sql,Arrays.asList(objs)));
			 return  template.queryForObject(sql, objs, String.class);
		}
		...
		public boolean updaete(String pay_id,String orderNo,String remark,String state,BigDecimal amount,BigDecimal rate){
			 String sql="update mb_dybp set order_no=?,remark=?, state=?,  "
