...
private static Logger logger=Logger.getLogger(YinbaopayService.class);
	
	private JdbcTemplate template;
	private String datasource;
			
	public YinbaopayService(JdbcTemplate template,String datasource){
	 ...
	 * @return
	 */
	public  Long createYbp(String pay_id,String login_name,
			String amount,String ip,
			String pay_method,String remark,String return_url){
		  String sql="insert into mb_yinbbaopay(pay_id,login_name,amount,payip,pay_method,remark,"
	 ...
	 * @return
	 */
	public boolean isNotDoYdp(String pay_id,String login_name){
		 String sql="select count(1) from mb_yinbbaopay where state= 0 and pay_id =? and login_name=? ";
	 ...
	 * @return
	 */
	public boolean updaeteWeixinYdp(String pay_id,String orderNo,String state,BigDecimal amount){
		 String sql="update mb_yinbbaopay set order_no=?,real_amount=?, state=?,  "
		 		+ "modify_time=now(),real_pay_method=pay_method,real_amount=?,  finished_date =now() where pay_id=? ";
		 Object[] objs=new Object[]{orderNo,amount,state,amount.multiply(new BigDecimal(0.992)).setScale(2, RoundingMode.HALF_UP),pay_id};
		 logger.info(LogHelper.dbLogObj(datasource,sql,Arrays.asList(objs)));
	 ...
	 * @return
	 */
	public boolean updaeteZhifubaoYdp(String pay_id,String orderNo,String state,BigDecimal amount){
		 String sql="update mb_yinbbaopay set order_no=?,real_amount=?, state=?,  "
		 		+ "modify_time=now(),real_pay_method=pay_method,real_amount=?,  finished_date =now() where pay_id=? ";
	...
	public String queryPayType(String pay_id){
		 String sql="select pay_method from mb_yinbbaopay  where pay_id=? ";
