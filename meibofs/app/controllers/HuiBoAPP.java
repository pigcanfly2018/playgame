...
import models.HuiBo;
import models.HuiBoRowMap;
import models.TongHui;
import models.TongHuiRowMap;
...
public class HuiBoAPP extends Controller{
	...
	public static void getList(int start,int limit,int page,String sdate,String edate,String sort,String queryKey)throws Exception{
		Sqler sql =new Sqler("select * from mb_hbp");
		Sqler cntsql =new Sqler("select count(1) from mb_hbp");
		...
		List<HuiBo> roleList=Sp.get().getDefaultJdbc().query(PageUtil.page(sql.getSql(),start,limit),sql.getParams().toArray(new Object[0]),new HuiBoRowMap());
