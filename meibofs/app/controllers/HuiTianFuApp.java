...
import models.HuiTianFu;
import models.HuiTianFuRowMap;
import models.User;
import models.YinBao;
import models.YinBaoRowMap;
import models.YingBao;
import models.YingBaoRowMap;
...
public class HuiTianFuApp extends Controller{
	...
	public static void getList(int start,int limit,int page,String sdate,String edate,String sort,String queryKey)throws Exception{
		Sqler sql =new Sqler("select * from mb_htp");
		Sqler cntsql =new Sqler("select count(1) from mb_htp");
		...
		List<HuiTianFu> roleList=Sp.get().getDefaultJdbc().query(PageUtil.page(sql.getSql(),start,limit),sql.getParams().toArray(new Object[0]),new HuiTianFuRowMap());
	...
	public static void success(String idcode){
		...
		if(HuiTianFu.NTupdatePwd(idcode)){
