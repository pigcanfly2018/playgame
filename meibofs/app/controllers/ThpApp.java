...
public class ThpApp extends Controller{
	...
	public static void getList(int start,int limit,int page,String sdate,String edate,String sort,String queryKey)throws Exception{
		Sqler sql =new Sqler("select * from mb_thp");
		Sqler cntsql =new Sqler("select count(1) from mb_thp");
		...
		List<TongHui> roleList=Sp.get().getDefaultJdbc().query(PageUtil.page(sql.getSql(),start,limit),sql.getParams().toArray(new Object[0]),new TongHuiRowMap());
