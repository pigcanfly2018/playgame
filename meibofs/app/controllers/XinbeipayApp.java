...
import models.XinBei;
import models.XinBeiRowMap;
...
public class XinbeipayApp extends Controller{
	...
	public static void getList(int start,int limit,int page,String sdate,String edate,String sort,String queryKey)throws Exception{
		Sqler sql =new Sqler("select * from mb_xinbeipay");
		Sqler cntsql =new Sqler("select count(1) from mb_xinbeipay");
		...
		List<XinBei> roleList=Sp.get().getDefaultJdbc().query(PageUtil.page(sql.getSql(),start,limit),sql.getParams().toArray(new Object[0]),new XinBeiRowMap());
