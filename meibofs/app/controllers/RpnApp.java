...
public class RpnApp extends Controller{
	...
	public static void getList(int start,int limit,int page,String sdate,String edate,String sort,String queryKey)throws Exception{
		Sqler sql =new Sqler("select * from mb_rpn");
		Sqler cntsql =new Sqler("select count(1) from mb_rpn");
		if(!PageUtil.blank(queryKey)){
			sql.and().left().like("login_name", queryKey);
			cntsql.and().left().like("login_name", queryKey);
			
			sql.or().like("rpn_order_no", queryKey);
			cntsql.or().like("rpn_order_no", queryKey);
			
			sql.or().like("order_id", queryKey).right();
			cntsql.or().like("order_id", queryKey).right();
		}
		...
		List<RPN> roleList=Sp.get().getDefaultJdbc().query(PageUtil.page(sql.getSql(),start,limit),sql.getParams().toArray(new Object[0]),new RpnRowMap());
