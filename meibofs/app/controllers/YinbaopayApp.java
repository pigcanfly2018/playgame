...
public class YinbaopayApp extends Controller{
	...
	public static void getList(int start,int limit,int page,String sdate,String edate,String sort,String queryKey)throws Exception{
		Sqler sql =new Sqler("select * from mb_yinbbaopay");
		Sqler cntsql =new Sqler("select count(1) from mb_yinbbaopay");
		...
		List<YinBao> roleList=Sp.get().getDefaultJdbc().query(PageUtil.page(sql.getSql(),start,limit),sql.getParams().toArray(new Object[0]),new YinBaoRowMap());
